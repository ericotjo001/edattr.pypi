...
def prepare_data(**kwargs):
    ...
    # Load our raw data - see above (assume it's in the form of csv file)
    import pandas as pd
...
def smoking_standard_train_val_test_endorse(parser):
    parser.add_argument('--toggle', default=None, type=str, help=None)  
    parser.add_argument('--n_epochs', default=8, type=int, help=None)  
    ...
    cf = classifierSmoking(DIRS, **kwargs)
...
def smoking_standard_aggregate(parser):
    print('smoking_standard_aggregate...')
    args, unknown = parser.parse_known_args()
    kwargs = vars(args)  # is a dictionary   
    kwargs.update({'prefix':'branch','label': 'dummylabel'})   

    label_suffix ='smoking_standard'
...
def smoking_compare_aggregate(parser):
    print("smoking_compare_aggregate...")
    parser.add_argument('--best.metric', default='acc', type=str, help=None)
    parser.add_argument('--label_suffix', default='smoking_standard', type=str, help=None)
...
if __name__=='__main__':
    ...
    parser.add_argument('--DATA_FILE_NAME', default='smoking.csv', type=str, help=None)
    ...
    parser.add_argument('--full_projectname', default='smoking', type=str, help=LABEL_INFO)
    ...
    if kwargs['mode'] == 'preprocess_normalize':
        ...
        smoking_standard_train_val_test_endorse(parser)
    elif kwargs['mode'] == 'standard_aggregate':
        smoking_standard_aggregate(parser)
    elif kwargs['mode'] == 'compare':
        smoking_compare(parser)   
    elif kwargs['mode'] == 'aggregate_compare':
        smoking_compare_aggregate(parser)        
    else:
