...
NUMERICAL_FEATURES = [ 'Cholesterol', 'triglyceride', 'HDL', 'LDL', 'hemoglobin', 'serum creatinine', 'AST', 'ALT', 'Gtp' ] # 'ID',  'weight(kg)', 'waist(cm)', 'eyesight(left)', 'eyesight(right)', 'systolic', 'relaxation', 'fasting blood sugar',
TOKEN_FEATURES = ['hearing(left)', 'hearing(right)', 'Urine protein', 'oral', 'dental caries', 'tartar'] # 'gender', 'age', 'height(cm)', 
TARGET_LABEL_NAME = 'smoking'
...
def myConfig(**kwargs):
    ...
    config = {
        ...
        'early_stopping':{
            'min_train_iters': 256,
            'val_every_n_iters':64, 
            'metrics_target': {'acc': 0.8, 'recall': 0.7,},
        },
...
def get_config_by_label(**kwargs):
    ...
    if suffix == 'smoking_standard_mlp_0000':
        layers = {'nD': 17, 'encoder_out_d': 21, 'fc': [37, 37, 27, C], }
        return myConfig(model='MLPEmb', layers=layers, dict_leng=dict_leng, **kwargs)
    elif suffix == 'smoking_standard_mlp_0001':
        layers = {'nD': 17, 'encoder_out_d': 27, 'fc': [57, 37, 27, C], }
        return myConfig(model='MLPEmb', layers=layers, dict_leng=dict_leng, **kwargs)
    elif suffix == 'smoking_standard_mlp_0002':
        layers = {'nD': 17, 'encoder_out_d': 37, 'fc': [67, 57, 27, C], }
        return myConfig(model='MLPEmb', layers=layers, dict_leng=dict_leng, **kwargs)  
    elif suffix == 'smoking_standard_resnet_0000': 
        ...
        iL_settings, emb_setting = make_intermediate_layer_settings_eResNetEmb(
            planes=[1, 1], n_blocks=[1,1], strides=[1,2], nD=3, encoder_out_d=5
        )        
        # C: no. of classes
    ...
    elif suffix == 'smoking_standard_resnet_0001': 
        from edattr.model import make_intermediate_layer_settings_eResNetEmb   
        iL_settings, emb_setting = make_intermediate_layer_settings_eResNetEmb(
            planes=[2, 4], n_blocks=[1,2], strides=[1,2], nD=5, encoder_out_d=11
        )        
        # C: no. of classes
        layers = {'iL_settings': iL_settings, 'emb_setting':emb_setting, 'C': C,}
        return myConfig(model='ResNetEmb', layers=layers, dict_leng=dict_leng, **kwargs)
    elif suffix == 'smoking_standard_resnet_0002': 
        from edattr.model import make_intermediate_layer_settings_eResNetEmb           
        iL_settings, emb_setting = make_intermediate_layer_settings_eResNetEmb(
            planes=[4, 8], n_blocks=[2,2], strides=[1,2], nD=7, encoder_out_d=17
        )        
        # C: no. of classes
        layers = {'iL_settings': iL_settings, 'emb_setting':emb_setting, 'C': C,}
        return myConfig(model='ResNetEmb', layers=layers, dict_leng=dict_leng, **kwargs)
    elif suffix == 'smoking_standard_transformer_0000':
        tf_conf = {'nhead':4, 'n_enc':2, 'nD': 12,}
        layers = {'C':C, 'dim_ff':48}
        return myConfig(model='TransformerEmb', layers=layers, tf_conf=tf_conf, dict_leng=dict_leng, **kwargs)
    elif suffix == 'smoking_standard_transformer_0001':
        tf_conf = {'nhead':4, 'n_enc':2, 'nD': 12,}
        layers = {'C':C, 'dim_ff':80}
        return myConfig(model='TransformerEmb', layers=layers, tf_conf=tf_conf, dict_leng=dict_leng, **kwargs)   
    elif suffix == 'smoking_standard_transformer_0002':
        tf_conf = {'nhead':4, 'n_enc':3, 'nD': 15,}
        layers = {'C':C, 'dim_ff':92}
        return myConfig(model='TransformerEmb', layers=layers, tf_conf=tf_conf, dict_leng=dict_leng, **kwargs)        
    else:
        from edattr.factory import clean_up_directory
        clean_up_directory(**kwargs)  # let's do a bit of cleanup
...
class classifierSmoking(StandardClassifierS2):
    def __init__(self, DIRS, **kwargs):
        ...
        super(classifierSmoking, self).__init__(DIRS, **kwargs)
